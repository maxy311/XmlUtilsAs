...
public class XlsUtils {
    ...
    public static void main(String[] args) {
        ...
        for (File valueDir : deskDir.listFiles()) {
            File file = new File(valueDir, "0918_Add.xls");
            if (file.isHidden())
                continue;
            String absolutePath = file.getAbsolutePath();
            if (!file.exists()) {
                System.out.println("wwwwww   " + absolutePath);
                continue;
            }
            System.out.println(absolutePath);

            Sheet sheet = loadExcel(absolutePath);
            if (sheet == null) {
                System.out.println("get sheet is empty : " + absolutePath);
                continue;
            }
            ...
            File outFile = new File(valueDir, "share_strings.xml");
            outPutFile(outFile, allRow);
